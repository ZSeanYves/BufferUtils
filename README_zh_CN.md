...
和基于 mmap 的 `NativeByteView` 研究，并且这条研究线现在采用
MoonBit-managed external owner bridge，带有显式 `close()` 和 finalizer
兜底。
...
Unix-like target 上的只读 mmap-backed access。它的 owner 是 MoonBit 管理的
external object，而不是稳定的 MoonBit `BytesView` bridge。
...
- 它需要显式 `close()`
- 它没有可预测的 automatic destructor 契约；finalizer 只作为兜底
- 它没有线程安全保证
...
例如 `Int` 或 `Bool`，或者通过 `copy_range(...)` 显式请求复制。owner 本身是
MoonBit-managed 的，所以即使忘记显式 `close()`，runtime finalizer 也能兜底释放。
...
| Case | 10MB 本地观测 | 解释 |
|---|---:|---|
| `native_file_source_read_chunk_experimental` | ~2656 MiB/s | native chunked read path，受 filesystem cache 明显影响 |
...
| `native_mmap_view_find_byte_experimental` | ~3363 MiB/s | C-side mmap scan，不会物化大块 MoonBit array |
| `native_mmap_view_count_byte_experimental` | ~12139 MiB/s | C-side full scan/count，对 cache 非常敏感 |
| `native_mmap_view_checksum_experimental` | ~922 MiB/s | mmap-backed memory 上的 C-side checksum |
| `native_mmap_view_crc32_experimental` | ~150 MiB/s | C-side CRC32 checksum |
| `native_mmap_view_copy_range_explicit_copy` | ~2827 MiB/s | 从 native view 显式复制到 MoonBit array |
| `native_mmap_view_copy_to_file_experimental` | ~4046 MiB/s | native-side transfer 到另一个文件路径 |
